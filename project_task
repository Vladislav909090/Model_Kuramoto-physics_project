...
theta_points = np.random.uniform(0, 2*np.pi, size=(N, 1))
omega_points = np.random.uniform(np.pi/4, np.pi, size=(N, 1))
# меняем значения под условие задачи
omega_points[0] = 10*3.6/R
omega_points[1] = 12*3.6/R

# создаем массив связей между точками
K_mas = np.random.uniform(1, 3, size=(N, N))
K_mas[0][0]=0
K_mas[0][1]=0.5
K_mas[1][0]=0
K_mas[1][1]=0
...
points = ax1.scatter([], [], color='red', s=50)


ax2.set_xlim(0, time_intervals[-1])  # Границы по оси x
ax2.set_ylim(-3, 3)  # Границы по оси y
...
def update(frame):
    global theta_points, mas_of_colors
    x = []
    y = []
...
ani.save(filename="project_ani_6.gif", writer="pillow")
